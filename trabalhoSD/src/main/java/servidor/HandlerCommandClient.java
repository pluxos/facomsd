			...
			for (String s : menu) {
				if (s.toLowerCase().equals(comando)) {
					...
					if (command_complete.split(" ").length > 1) {
						if (comando.equals("create") || comando.equals("update") || comando.equals("1")
								|| comando.equals("3")) {
							if (command_complete.split(":").length < 1) {
								return false;
							}
							...
							if (!command_complete
									.substring(command_complete.indexOf(" ") + 1, command_complete.indexOf(":") - 1)
									.matches(regex_num)) {
								return false;
							}
						...
						if (comando.equals("read") || comando.equals("delete") || comando.equals("2")
								|| comando.equals("4")) {
							String value = command_complete.substring(command_complete.indexOf(" ") + 1,
									command_complete.length());
