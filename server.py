...
Tempo de Snapshot(segundos):30'''
    ...
    def lineOneChordMode(self):
        ...
        auxKey = self.chordId - self.espectrum
        ...
        while True:
            ...
            elif key <= self.chordId and key > auxKey:
    ...
    def lineFour(self):
        ...
        while True:
            ...
            if key <= lowerKey and key >= halfChord:
                channel = grpc.insecure_channel(self.routingTable['prev'])
                stub = database_pb2_grpc.DatabaseOperationStub(channel)
                self.handleRequest(stub,request)
                
            else:
                channel = grpc.insecure_channel(self.routingTable['next'])
                stub = database_pb2_grpc.DatabaseOperationStub(channel)
                self.handleRequest(stub,request)
    ...
    def handleRequest(self, stub, request):
        ...
        if   op == '1':
            ...
            #self.assyncResponse(response,request[1])
            ...
            response = stub.Update(database_pb2.CrUpRequest(chave=key,dados=value))
            #self.assyncResponse(response,request[1])
            request[1].put(response.resposta)
        elif op == '4':
    ...
    def registerOnChord(self,registerId):
        ...
        self.routingTable = {}
        self.routingTable['prev'] = self.host + ':' + str(50050 + idPrev)
        self.routingTable['next'] = self.host + ':' + str(50050 + idNext)
        self.onChord = True

        print('On chord, keys:' ,self.chordId,',', self.chordId - self.espectrum)

    #def assyncResponse(self,future,responseLine):
        #response = future.result()
        #responseLine.put(response.resposta)
