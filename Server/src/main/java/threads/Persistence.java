		...
		try {
			while (true) {
				...
				if (new String(obj.getControll()).equals("CREATE")) {
					callback = ( Database.Insert(new BigInteger(obj.getKey()), obj.getValue()) ) ? "CREATE SUCESS!" : "CREATE FAIL!";
				}
				else if (new String(obj.getControll()).equals("UPDATE")) {
					callback = ( Database.Update(new BigInteger(obj.getKey()), obj.getValue()) ) ? "UPDATE SUCESS!" : "UPDATE FAIL!";
				}
