...
public class Logger implements Runnable {
    ...
    public void getListOfCommands() {
        ...
        ItemFila item;
        // System.out.println("MEGA BATATA");
        try {
            contents = Files.readAllLines(path);
            for (String content : contents) {
                String[] commandSplited = content.split("\\s+");
                if (commandSplited.length == 3) {
                    item = new ItemFila(null, Controll.valueOf(commandSplited[0]), new BigInteger(commandSplited[1]),
                            commandSplited[2].getBytes());
                    // item = new ItemFila( null, commandSplited[0].getBytes(), new BigInteger(
                    // commandSplited[1] ).toByteArray(), commandSplited[2].getBytes());
                    // item.print();
                    f3.add(item);
