package br.ufu;

import org.junit.Test;
import org.junit.runner.RunWith;
import org.mockito.Mockito;
import org.mockito.junit.MockitoJUnitRunner;
import org.mockito.stubbing.Answer;

import java.math.BigInteger;
import java.util.List;
import java.util.Scanner;
import java.util.concurrent.ArrayBlockingQueue;
import java.util.concurrent.BlockingQueue;

import static br.ufu.TestUtil.*;
import static org.mockito.Mockito.when;

@RunWith(MockitoJUnitRunner.class)
public class CrudOverLimitTest extends BaseTest {

    @Test
    public void shouldTestCrudOverLimit() throws Exception {

        deleteLogsAndSnapshots();
        deleteAtomixLogs();

        Integer m = 4;

        List<Thread> servers =  initServers(m, 4, 4444, 100000, 3);
        BigInteger value = new BigInteger("2").pow(m).subtract(new BigInteger("1"));

        for (Thread thread: servers) {
            thread.start();
            Thread.sleep(100);
        }
        Thread.sleep(1000);

        String[] commands = getClientArgs(4445);
        Client clientSpy = Mockito.spy(new Client(commands));

        Scanner mockScanner = Mockito.mock(Scanner.class);

        BlockingQueue<String> inputs = new ArrayBlockingQueue<>(50);

        inputs.offer("CREATE "+ value.add(new BigInteger("1")) +" X");
        inputs.offer("READ "+ value.add(new BigInteger("100")));
        inputs.offer("UPDATE "+ value.add(new BigInteger("21")) +" Y");
        inputs.offer("DELETE "+ value.add(new BigInteger("2311")));
        inputs.offer("sair");

        when(clientSpy.getScanner()).thenReturn(mockScanner);
        when(mockScanner.hasNext()).thenAnswer((Answer<Boolean>) invocation -> true);
        when(mockScanner.nextLine()).thenAnswer((Answer<String>) invocation -> {
            Thread.sleep(3000);
            return inputs.take();
        });

        Thread.sleep(3000);

        Thread tClient = getThread(clientSpy);
        System.out.println("Client started!");
        tClient.start();
        tClient.join();

        verifyMessage("Key not supported. Accepted range: [ 0 , 15 ]");
        verifyMessage("Key not supported. Accepted range: [ 0 , 15 ]");
        verifyMessage("Key not supported. Accepted range: [ 0 , 15 ]");
        verifyMessage("Key not supported. Accepted range: [ 0 , 15 ]");

        for (Thread thread: servers) {
            thread.stop();
        }
    }


}
