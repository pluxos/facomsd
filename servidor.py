...
def inicia_server(): #conecta ao cliente e inicia thread para receber requisicoes
    ...
    ativo = True
    while ativo:
        cliente, endereco = soc.accept()
        cliente.settimeout(30)
        cont += 1
        print("Conectado com: " + str(endereco[0]) + ":" + str(endereco[1]))
        try:
            Thread(target=client_thread, name="client", args=(cliente, endereco, cont)).start()
        except:
...
def client_thread(cliente, endereco, cont): #recebe as requisicoes dos clientes e inicia thread para duplicar a fila
    ativo = True
    while ativo:
        try:
            requisicao = cliente.recv(1024).decode("utf8")
            if requisicao:
                ...
                if int(temp[0]) == 5:
                    ...
                    cont -= 1
                    if cont == 0:
                        encerra_server() #se todos os clientes encerrarem a cliente o servidor encerra
                    cliente.close()
                    ativo = False
                else:            
                    try:
                        f1.insere(requisicao)
                        d = Thread(target=duplica_thread, name="duplica", args=(cliente, endereco))
                        d.start()
                    except:
...
def duplica_thread(cliente, endereco):# remove as requisicoes de f1 e duplica para f2 e f3 e inicia as threads de log e banco
    ativo = True
    while ativo:
        while not f1.vazia():
            comando = f1.retira()
            ...
            f3.insere(comando)
            try:
                l = Thread(target=log_thread, name="log",args=(cliente, endereco))
                d = Thread(target=banco_thread, name="database",args=(cliente, endereco))
                l.start()                
                d.start()
            except:
...
def log_thread(cliente, endereco): #grava as requisicoes em arquivo de log
    ativo = True
    log = open("log.txt","a")
    while ativo:
        while not f2.vazia():
...
def banco_thread(cliente, endereco): # grava as requisicoes em banco de dados
    ativo = True
    while ativo:
        while not f3.vazia():
            cm = str(f3.retira())
            ...
            if int(cm[0]) == 1 :
                if bd.create(int(cm[1]),cm[2]):
                    cliente.sendall("OK".encode("utf8"))
                    ok = True
            elif int(cm[0]) == 2 :
                    read = bd.read(int(cm[1]))
                    if read:
                        cliente.sendall(("Chave: " + str(cm[1]) +" Valor: " + read).encode("utf8"))
                        ok = True
            elif int(cm[0]) == 3 :
                ...
                if bd.delete(int(cm[1])):
                    ...
                    ok = True
            if not ok:
                cliente.sendall("NOK".encode("utf8"))
        ativo = False
