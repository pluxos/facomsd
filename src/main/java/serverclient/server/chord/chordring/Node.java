...
public class Node extends Thread implements Comparable<Node> {
	...
	public Node(String name, String port, int size, int replicasNumbers) {
		myname = name;
		ring_size = size;
		myport = Integer.parseInt(port);
		myid = this.createNodeId(seira, ring_size);
		global_rep = replicasNumbers;
	}

	public Node(String name, int size, int replicasNumbers) {
		myname = name;
		ring_size = size;
		myport = nextPort;
		nextPort++;
	...
	private boolean isResponsibleForId (int song_id){
	...
	int query(String key,int counter) throws NoSuchAlgorithmException{
		int song_id = this.createNodeId(key, ring_size);
		int value = -1; // i am not responsible for song
		if (isResponsibleForId(song_id) ){
	...
	int delete (String key,int counter) throws NoSuchAlgorithmException{
		...
		if (!isResponsibleForId(song_id) && counter < global_rep) {
			files.remove(key);
			answer = -3;
		}
		if (counter == 1){
...
//DEBUGGING     			System.out.println(queryresult);
	        	...
	        	if (myid == hisSuccessorID ){
	        		for (String key : files.keySet()) {
						...
						if (this.createNodeId(key, ring_size) <= predecessor.getmyId()){
	...
	private int createNodeId(String nodeKey, int ringSize) {
		...
		byte[] nodeKeyBytes = mDigest.digest(nodeKey.getBytes());

		StringBuffer sb = new StringBuffer();

		for (int i = 0; i < nodeKeyBytes.length; i++) {
			// Transformar em hexa
			sb.append(String.format("%02x", nodeKeyBytes[i]));
		}

		BigInteger value = new BigInteger(sb.toString(), 16);

		value = value.mod(BigInteger.valueOf(ringSize));
